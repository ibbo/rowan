...
ALLOWED_KEY_TERMS = {
    "scottish",
    "scd",  # common abbreviation for Scottish Country Dancing
    "scddb",
    "rscds",
    "ceilidh",
    "strathspey",
...
def _is_scottish_country_dance_query(text: str) -> bool:
    """Return True if the prompt is about Scottish Country Dancing topics."""
    lowered = text.lower()

    # Direct keyword/phrase matches that clearly indicate Scottish Country Dancing
    for term in ALLOWED_KEY_TERMS:
        if term in lowered:
            return True

    # Allow generic references when both 'scottish' and 'dance' appear
    if "scottish" in lowered and "dance" in lowered:
        return True

    # Accept references to planning classes or dances that mention Scottish context implicitly
    if re.search(r"\b(plan|planning|organize|schedule)\b", lowered) and (
        "scd" in lowered or "country dance" in lowered or "rscds" in lowered
    ):
        return True
...
def _extract_user_question(message: HumanMessage) -> str:
    """Best-effort extraction of the user's actual question text."""
    ...
    match = re.search(r"user\s+(?:question|input|prompt)\s*:\s*(.+)", text, flags=re.IGNORECASE | re.DOTALL)
    if match:
...
def _guard_pre_model_hook(state: MessagesState) -> Command | Dict[str, Any]:
    """Inspect the latest human message before the LLM runs."""
    for message in reversed(state.get("messages", [])):
        if isinstance(message, HumanMessage):
            user_text = _extract_user_question(message)
            if _is_scottish_country_dance_query(user_text):
                return {}
            refusal_text = (
...
async def create_dance_agent():
    ...
    agent = create_react_agent(
        ...
        pre_model_hook=_guard_pre_model_hook,
    )
